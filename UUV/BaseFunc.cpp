...
const static UUV_U16 CRC16Table[] = 
        { 
...
///计算数组的CRC校验码
///</summary>
///<param name="crc">初始计算的校验码</param>
///<param name="start">计算CRC起始位置</param>
///<param name="len">计算数据长度</param>
///<param name="buf">计算数据数组</param>
...
UUV_U16 Udp_CalcCRC16(UUV_U16 crc, UUV_U16 start, UUV_U16 len, UUV_U8* buf)
 {
	...
	for (i = 0; i<len; i++)
     {
		crc = (UUV_U16)(CRC16Table[(crc ^ buf[start + i]) & 0xff] ^ (crc>> 8));
     }
	return (crc);
 }
